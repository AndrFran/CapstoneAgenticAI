...
CHANGELOG = {
    ...
    "inventory": [
        ("v1", "Initial: stock positions, shortages, transfers."),
        (
            "v2",
            "Added a decision tree (position -> required quantity -> transfers), "
            "mandatory vocabulary, and a fixed finding skeleton carrying "
    ...
    "supplier": [
        ("v1", "Initial: supplier profiles, alternatives, comparison, costing."),
        (
            "v2",
            "Added a decision tree (details -> alternatives -> compare -> cost), "
            "upstream quantity rules, explicit boundaries, and a finding template "
            "with an alternatives table. Live runs showed the agent costing "
            "before comparing, omitting exclude_supplier_id, and inventing a buy "
            "size when the operator had given none - it now reports "
            "'missing quantity - cannot cost' instead. (TM3, from trace "
            "insights.)",
        ),
    ],
}

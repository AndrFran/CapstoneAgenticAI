...
| `studio.py` | The LangGraph Server entry point named by `langgraph.json` — `graph.build_workflow()` compiled **without** a checkpointer, because the server owns persistence there. Only `langgraph dev` / LangGraph Platform load it; Streamlit and the scripts go through `runner`. `docs/langgraph-studio.md`. |
